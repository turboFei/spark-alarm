...
class StreamingAlarmListenerTest extends FunSuite {
  ...
  test("test get limit failure reason") {
    ...
    assert(getLimitFailureReason(reason0) === reason0)
    assert(getLimitFailureReason(reason1) === reason1)
    assert(getLimitFailureReason(reason2) === reason2)
    assert(getLimitFailureReason(reason3) === reason3.split("\n")(0))

    setStaticFieldValue(className, factorFieldName, 1.1)
    assert(getLimitFailureReason(reason2) === reason2.substring(0, (maxLengthFactor * failureReasonLimit).toInt))
  }
